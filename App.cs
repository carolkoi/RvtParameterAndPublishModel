...
namespace RevitParametersAddin
{
    ...
    {
        const string RIBBON_TAB = "ACC & Revit";
        const string RIBBON_PANEL = "ACC & Revit Panel";
        public Result OnStartup(UIControlledApplication a)
        {
            ...
            PushButtonData publishBtn = new PushButtonData(
                "Publish Model",
                "Publish Model",
                Assembly.GetExecutingAssembly().Location,
                "RevitParametersAddin.PublishCommand"
                )
            {
                ToolTip = "Button to publish Revit model in ACC",
                LongDescription = "Publish Revit Model in ACC",
                Image = imgSrc,
                LargeImage = imgSrc
            };
            PushButton btn = panel.AddItem(btnData) as PushButton;
            PushButton btn2 = panel.AddItem(publishBtn) as PushButton;
            btn.Enabled = true;
            btn2.Enabled = true;
